...
void OccGrid::insertPanorama(const std::string bagfile)
{
  ...
  for (auto m : rosbag::View(bag, rosbag::TopicQuery(topics))) {
    ...
    if (!camera_pose.empty() && !imgs.empty()) {
      ...
      pose.x = camera_pose.front().pose.position.x;
      pose.y = camera_pose.front().pose.position.y;
      pose.theta = tf::getYaw(camera_pose.front().pose.orientation);
      ...
      tf::Quaternion quat;
      tf::quaternionMsgToTF(camera_pose.front().pose.orientation, quat);
  ...
  Point po(pan_pose.pose.position.x, pan_pose.pose.position.y);
  updateRobotCells(po);
